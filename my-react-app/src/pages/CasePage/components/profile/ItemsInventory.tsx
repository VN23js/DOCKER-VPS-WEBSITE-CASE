import type { ProfileUser } from '../../../../redux/TypeSlice/sliceTypeCase';

type Props = {
  profileUser: ProfileUser | null;
};

export default function ItemsInventory({ profileUser }: Props) {
  return (
    <div className='grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3'>
      {profileUser?.inventory.map((item, i) => (
        <div
          key={`${item._id}-${i}`}
        ...
        >
          ...
          <div className='relative'>
            ...
            <div className='item_title flex text-[13px]'>{item.nameWeapon}</div>
            <p className='text-[13px] truncate !font-geo'>{item.nameSkin}</p>
            <p
